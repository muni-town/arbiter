...
#     cargo build -p arbiter-server --release --target $TARGET

RUN cargo build -p arbiter-server --release --target $TARGET

FROM alpine:3
ARG TARGET
COPY --from=build /project/target/${TARGET}/release/arbiter-server /arbiter-server
